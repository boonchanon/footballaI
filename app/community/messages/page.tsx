...
import { ArrowLeft, Edit3, ImagePlus, Loader2, MessageSquare, Newspaper, Search, Send, X } from "lucide-react"
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto max-w-7xl px-4 py-20">
        ...
        {!token ? (
          ...
          <div className="grid min-h-[72vh] gap-5 lg:grid-cols-[380px_minmax(0,1fr)]">
            <Card className="overflow-hidden rounded-[28px] border-border/60 bg-card/80 shadow-[0_20px_50px_rgba(0,0,0,0.18)]">
              <CardContent className="p-0">
                <div className="border-b border-border/60 px-5 py-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <h1 className="text-xl font-semibold">ข้อความ</h1>
                      <p className="text-sm text-muted-foreground">แชตรอคุยและบทสนทนาล่าสุด</p>
                    </div>
                    {totalUnreadMessages ? (
                      <div className="rounded-full bg-primary/15 px-3 py-1 text-xs font-semibold text-primary shadow-[0_0_20px_rgba(184,255,0,0.18)]">
                        ข้อความใหม่ {totalUnreadMessages > 99 ? "99+" : totalUnreadMessages}
                      </div>
                    ) : null}
                  </div>
                </div>
                ...
                <div className="border-b border-border/60 px-4 py-4">
                  <div className="relative mb-4">
                    ...
                    <Input
                      ...
                      placeholder="ค้นหาคนคุยหรือข้อความ..."
                      className="h-11 rounded-2xl border-border/60 bg-background/60 pl-11"
                    />
                  </div>
                  <div className="flex items-center gap-3 overflow-x-auto pb-1">
                    {filteredConversations.map((conversation) => {
                      ...
                      return (
                        <button
                          key={`bubble-${conversation.id}`}
                          onClick={() => setActiveConversationId(conversation.id)}
                          className="flex shrink-0 flex-col items-center gap-2"
                        >
                          <div
                            className={cn(
                              "relative rounded-full border-2 p-1 transition",
                              active ? "border-primary bg-primary/10" : "border-border/60 bg-background/50",
                              conversation.hasUnread && "border-primary/70 shadow-[0_0_0_4px_rgba(184,255,0,0.12),0_0_24px_rgba(184,255,0,0.28)]",
                            )}
                          >
                            ...
                            {conversation.unreadCount ? (
                              <span className="absolute -right-1 -top-1 flex h-5 min-w-5 items-center justify-center rounded-full bg-primary px-1 text-[10px] font-semibold text-primary-foreground shadow-[0_0_18px_rgba(184,255,0,0.55)]">
                                {conversation.unreadCount > 9 ? "9+" : conversation.unreadCount}
                              </span>
                            ) : null}
                          </div>
                          <span className={cn("max-w-[72px] truncate text-[11px]", conversation.hasUnread ? "font-medium text-foreground" : "text-muted-foreground")}>
                      ...
                      <button
                        ...
                        className={cn(
                          ...
                          activeConversationId === conversation.id
                            ? "border-primary/40 bg-primary/8 shadow-[0_10px_30px_rgba(184,255,0,0.08)]"
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center justify-between gap-2">
                            <div className="flex min-w-0 items-center gap-2">
                              <p className={cn("truncate text-sm", conversation.hasUnread ? "font-semibold text-foreground" : "font-medium")}>
                                {conversation.user.name}
                              </p>
                              {conversation.hasUnread ? <span className="h-2 w-2 rounded-full bg-primary shadow-[0_0_12px_rgba(184,255,0,0.6)]" /> : null}
                            </div>
                          ...
                          <div className="mt-2 flex items-start justify-between gap-3">
                            <p className={cn("line-clamp-2 text-xs", conversation.hasUnread ? "font-medium text-foreground" : "text-foreground/80")}>
            ...
            <Card className="overflow-hidden rounded-[28px] border-border/60 bg-card/80 shadow-[0_20px_50px_rgba(0,0,0,0.18)]">
              <CardContent className="flex h-full flex-col p-0">
                {currentConversation ? (
                  <>
                    <div className="sticky top-0 z-10 flex items-center justify-between gap-3 border-b border-border/60 bg-background/90 px-5 py-4 backdrop-blur">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">{currentConversation.user.favoriteTeam || currentConversation.user.bio || "เพื่อนในคอมมูนิตี้"}</p>
                        </div>
                      </div>
                    ...
                    <div className="flex-1 overflow-y-auto bg-[radial-gradient(circle_at_top,rgba(184,255,0,0.08),transparent_28%),linear-gradient(180deg,rgba(255,255,255,0.02),transparent)] px-4 py-5">
                              ...
                              <div key={message.id} className={cn("flex", isMine ? "justify-end" : "justify-start")}>
                                <div
                                  className={cn(
                                    ...
                                    isMine ? "rounded-br-md bg-primary text-primary-foreground" : "rounded-bl-md border border-border/60 bg-card",
                                  )}
                                >
                                  ...
                                  {message.sharedItem?.title ? (
                                    <a
                                      ...
                                      className={cn(
                                        ...
                                        isMine ? "border-white/15 bg-black/10" : "border-border/60 bg-muted/25",
                                      )}
                                    >
                                      <div className="flex items-center gap-3">
                                        {message.sharedItem.image ? (
                                          ...
                                          <div className={cn("flex h-16 w-16 shrink-0 items-center justify-center rounded-xl", isMine ? "bg-white/10" : "bg-muted")}>
                                            <Newspaper className="h-5 w-5 text-primary" />
                                          </div>
                                        )}
                                        <div className="min-w-0">
                                          <p className={cn("text-[11px]", isMine ? "text-primary-foreground/70" : "text-muted-foreground")}>
                                          ...
                                          {message.sharedItem.description ? (
                                            <p className={cn("mt-1 line-clamp-3 text-xs leading-5", isMine ? "text-primary-foreground/80" : "text-muted-foreground")}>
                                              {message.sharedItem.description}
                                            </p>
                                          ) : null}
                                          <p className={cn("mt-1 truncate text-[11px]", isMine ? "text-primary-foreground/70" : "text-muted-foreground")}>
                                  ...
                                  <div className={cn("mt-1 flex items-center gap-2 text-[11px]", isMine ? "justify-end text-primary-foreground/70" : "text-muted-foreground")}>
                                  ...
                                  {isMine && message.seenByRecipient && message.seenTimeAgo ? (
                                    <p className="mt-1 text-right text-[11px] text-primary-foreground/70">อ่านข้อความแล้วเมื่อ {message.seenTimeAgo}</p>
                                  ) : null}
                                </div>
                              </div>
                            )
                    ...
                    <div className="border-t border-border/60 bg-background/75 px-4 py-4 backdrop-blur">
                      <div className="mx-auto max-w-3xl space-y-3">
                        {pendingSharedItem ? (
                          <div className="rounded-2xl border border-primary/30 bg-primary/5 p-3">
                            ...
                            <a href={pendingSharedItem.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 rounded-2xl border border-border/60 bg-background/60 p-3">
                        ...
                        {attachedImages.length ? (
                          <div className="grid grid-cols-2 gap-2 rounded-2xl border border-border/60 bg-background/40 p-3">
                            {attachedImages.map((imageUrl) => (
                              <div key={imageUrl} className="relative overflow-hidden rounded-2xl border border-border/60">
                        ...
                        <div className="flex items-end gap-3">
                          <label className="inline-flex h-[52px] cursor-pointer items-center justify-center rounded-2xl border border-border/60 bg-background/70 px-4 text-muted-foreground transition hover:border-primary/40 hover:text-primary">
                          ...
                          <Textarea
                            ...
                            placeholder={pendingSharedItem ? "พิมพ์ความเห็นเพิ่มก่อนส่ง..." : "พิมพ์ข้อความถึงเพื่อน..."}
                            className="min-h-[52px] rounded-2xl border-border/60 bg-background/70"
                          />
                          <Button
                            onClick={sendMessage}
